-- Fix the type mismatch in get_advanced_member_analytics function
DROP FUNCTION IF EXISTS public.get_advanced_member_analytics(date, date, integer, integer, text[], text[], numeric, numeric, text[]);
...
BEGIN
  ...
  member_analytics AS (
    SELECT 
      ...
      mb.interests,
      mb.tier_badge,
      COUNT(ml.*)::bigint as total_transactions,
      COALESCE(SUM(CASE WHEN ml.amount > 0 THEN ml.amount ELSE 0 END), 0)::numeric as total_spend,
      COALESCE(AVG(CASE WHEN ml.amount > 0 THEN ml.amount ELSE NULL END), 0)::numeric as avg_transaction,
      MIN(ml.activity_date) as first_transaction_date,
      MAX(ml.activity_date) as last_transaction_date,
      COUNT(DISTINCT DATE_TRUNC('month', ml.activity_date))::bigint as active_months,
      COUNT(DISTINCT ml.activity_date)::bigint as active_days,
      ...
      COALESCE(SUM(CASE WHEN ml.activity_date >= DATE_TRUNC('month', NOW()) THEN ml.amount ELSE 0 END), 0)::numeric as current_month_spend,
      COALESCE(SUM(CASE WHEN ml.activity_date >= DATE_TRUNC('week', NOW()) THEN ml.amount ELSE 0 END), 0)::numeric as current_week_spend,
      COUNT(CASE WHEN ml.activity_date >= DATE_TRUNC('month', NOW()) THEN 1 END)::bigint as current_month_transactions
    FROM member_base mb
  ...
  venue_analytics AS (
    SELECT 
      ...
      CASE 
        ...
        THEN (COUNT(DISTINCT mc.check_in_date)::NUMERIC / GREATEST((MAX(mc.check_in_date) - MIN(mc.check_in_date))::INTEGER / 7.0, 1))::numeric
  ...
  SELECT 
    ...
    COALESCE(va.visit_frequency, 0)::numeric as visit_frequency,
    va.last_visit_date,
