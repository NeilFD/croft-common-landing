-- Update the get_member_deep_dive function to include individual items data
...
BEGIN
  ...
  -- Get individual items data from receipts
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      'times_ordered', item_data.times_ordered
    )
  ) INTO items_data
  FROM (
    SELECT 
      ...
      SUM((item->>'quantity')::numeric * (item->>'price')::numeric) as total_spent,
    ...
    ORDER BY SUM((item->>'quantity')::numeric * (item->>'price')::numeric) DESC
    LIMIT 10
  ) item_data;
