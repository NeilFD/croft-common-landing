-- Fix get_member_deep_dive function - resolve ambiguous column references
...
BEGIN
  ...
  -- Calculate simple weekly streaks based on receipt counts
  ...
  current_streak_calc AS (
    -- Simple approach: count consecutive completed weeks from most recent
    SELECT COUNT(*) as current_streak
    FROM (
      SELECT week_start, is_complete, ROW_NUMBER() OVER (ORDER BY week_start DESC) as rn
  ...
  longest_streak_calc AS (
    -- Simple approach: use existing member_streaks table as fallback
    SELECT COALESCE(MAX(ms.longest_streak), 1) as longest_streak
    FROM public.member_streaks ms
    WHERE ms.user_id = p_user_id
  )
  -- Get visit patterns with calculated streaks - FIX AMBIGUOUS REFERENCE
  SELECT jsonb_build_object(
    'total_visits', (SELECT COUNT(*) FROM public.member_receipts WHERE member_receipts.user_id = p_user_id),
    'current_streak', COALESCE((SELECT current_streak FROM current_streak_calc), 0),
    'longest_streak', COALESCE((SELECT longest_streak FROM longest_streak_calc), 0),
