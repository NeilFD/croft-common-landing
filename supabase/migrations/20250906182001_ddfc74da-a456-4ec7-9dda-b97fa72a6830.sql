-- Fix get_member_deep_dive function - use simple streak calculation
...
BEGIN
  ...
  longest_streak_calc AS (
    ...
    SELECT COALESCE(MAX(longest_streak), 1) as longest_streak
    FROM public.member_streaks
    WHERE user_id = p_user_id
  )
  -- Get visit patterns with calculated streaks
  SELECT jsonb_build_object(
