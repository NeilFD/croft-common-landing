-- Fix data type mismatches in get_advanced_member_analytics function
...
RETURNS TABLE(
  ...
  interests text[], 
  tier_badge text, 
  total_transactions bigint,  -- Changed from integer to bigint
  total_spend numeric, 
  avg_transaction numeric, 
  first_transaction_date date, 
  last_transaction_date date, 
  active_months bigint,  -- Changed from integer to bigint
  active_days bigint,  -- Changed from integer to bigint
  categories text[], 
  payment_methods text[], 
  currency text, 
  current_month_spend numeric, 
  current_week_spend numeric, 
  current_month_transactions bigint,  -- Changed from integer to bigint
...
BEGIN
  ...
  WITH base_data AS (
    ...
    SELECT 
      ...
      -- Use receipts data for accurate totals - no duplication
    ...
    HAVING 
      ...
      -- Include all members, even those without receipts for complete analytics
      AND 1=1
  ),
  enhanced_data AS (
    SELECT 
