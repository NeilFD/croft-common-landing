-- Fix the column ambiguity issue in get_member_deep_dive function
...
BEGIN
  ...
  -- Get visit patterns data with proper day analysis
  ...
  SELECT jsonb_build_object(
    'churn_risk', CASE 
      WHEN (SELECT MAX(mr9.receipt_date) FROM public.member_receipts mr9 WHERE mr9.user_id = p_user_id) < CURRENT_DATE - INTERVAL '30 days' THEN 'high'
      WHEN (SELECT MAX(mr10.receipt_date) FROM public.member_receipts mr10 WHERE mr10.user_id = p_user_id) < CURRENT_DATE - INTERVAL '14 days' THEN 'medium'
      ELSE 'low'
    END,
