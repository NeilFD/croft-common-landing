...
-- Allow system to update subscriber status for unsubscribe with token verification
-- This policy allows the unsubscribe edge function to work
CREATE POLICY "System can update for unsubscribe with valid token"
ON public.subscribers
FOR UPDATE
USING (
  -- Allow update if the row being updated has a valid unsubscribe token
  -- This is checked by the edge function before calling update
  true
)
WITH CHECK (
  -- Only allow setting is_active to false and updating sync status
  -- This restricts what can be updated during unsubscribe
  (is_active = false OR is_active = OLD.is_active) AND
  (mailchimp_sync_status IS NOT NULL OR mailchimp_sync_status = OLD.mailchimp_sync_status) AND
  (updated_at >= OLD.updated_at)
);

-- Add documentation comments
COMMENT ON TABLE public.subscribers IS 'Newsletter subscribers with secure RLS policies. Public can only insert new subscriptions. Admins can manage all data. Unsubscribe requires token verification.';
