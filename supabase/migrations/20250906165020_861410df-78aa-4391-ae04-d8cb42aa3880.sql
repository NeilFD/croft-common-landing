-- Enhanced get_member_deep_dive function with expanded visit patterns
...
BEGIN
  ...
  -- Get enhanced month period analysis
  SELECT jsonb_agg(
    jsonb_build_object(
      'period', period_name,
  ...
  FROM (
    SELECT 
      ...
      (SELECT COUNT(*) FROM public.member_check_ins WHERE user_id = p_user_id) as total_visits
  ...
  -- Get time of day categorization
  SELECT jsonb_agg(
    jsonb_build_object(
  ...
  -- Calculate visit consistency and gaps
  WITH visit_gaps AS (
    SELECT 
      check_in_date,
  ...
  SELECT jsonb_build_object(
    ...
    'most_likely_day_percentage', ROUND((day_count::numeric / NULLIF((SELECT COUNT(*) FROM public.member_check_ins WHERE user_id = p_user_id), 0) * 100), 1)
