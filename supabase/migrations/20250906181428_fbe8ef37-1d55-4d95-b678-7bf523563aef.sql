-- Update get_member_deep_dive function to calculate weekly streaks correctly
...
BEGIN
  ...
  streak_calculation AS (
    SELECT 
      ...
      -- Check if this week is consecutive to previous week
      CASE 
        WHEN LAG(week_start) OVER (ORDER BY week_start) = week_start - INTERVAL '1 week' AND 
             LAG(is_complete) OVER (ORDER BY week_start) = true AND 
             is_complete = true 
        THEN 0 
        ELSE 1 
      END as streak_break
  ...
  streak_groups AS (
    SELECT 
      week_start,
      is_complete,
      SUM(streak_break) OVER (ORDER BY week_start) as streak_group
  ...
  current_streak_calc AS (
    SELECT 
      CASE 
        WHEN MAX(wrc.week_start) >= DATE_TRUNC('week', CURRENT_DATE)::DATE - INTERVAL '1 week' 
             AND MAX(wrc.is_complete) = true
        THEN (
          SELECT COUNT(*)
          FROM weekly_receipt_counts wrc2
          WHERE wrc2.week_start <= (SELECT MAX(week_start) FROM weekly_receipt_counts WHERE is_complete = true)
            AND wrc2.week_start > COALESCE(
              (SELECT MAX(week_start) 
               FROM weekly_receipt_counts wrc3 
               WHERE wrc3.week_start < (SELECT MAX(week_start) FROM weekly_receipt_counts WHERE is_complete = true)
                 AND wrc3.is_complete = false), 
              '1900-01-01'::DATE
            )
            AND wrc2.is_complete = true
        )
        ELSE 0
      END as current_streak
    FROM weekly_receipt_counts wrc
  )
  SELECT jsonb_build_object(
    'current_streak', COALESCE(csc.current_streak, 0),
    'longest_streak', COALESCE((SELECT MAX(streak_length) FROM streak_lengths), 0),
    'total_weeks_completed', COALESCE((SELECT COUNT(*) FROM weekly_receipt_counts WHERE is_complete = true), 0)
  ) INTO weekly_streak_data
  FROM current_streak_calc csc;
  ...
  -- Get enhanced visit patterns using RECEIPT-based data with CORRECTED weekly streaks
  SELECT jsonb_build_object(
    'total_visits', COUNT(mr.id),
    'current_streak', (weekly_streak_data->>'current_streak')::INTEGER,
    'longest_streak', (weekly_streak_data->>'longest_streak')::INTEGER,
