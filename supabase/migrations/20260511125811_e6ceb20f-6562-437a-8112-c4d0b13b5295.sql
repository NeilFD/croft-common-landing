...
CREATE OR REPLACE FUNCTION public.create_cinema_booking(_user_id uuid, _email text, _primary_name text, _guest_name text, _quantity integer)
 RETURNS TABLE(ticket_numbers integer[], release_id uuid, wallet_token uuid, booking_id uuid)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  RETURNING cinema_bookings.id, cinema_bookings.wallet_token INTO v_booking_id, v_token;

  RETURN QUERY SELECT v_tickets, v_release_id, v_token, v_booking_id;
END;
$function$;