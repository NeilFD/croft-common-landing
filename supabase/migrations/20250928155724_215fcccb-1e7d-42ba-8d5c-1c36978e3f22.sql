-- Fix the generate_contract function to use the correct table name
...
BEGIN
  ...
  -- Build line items content and calculate totals using CORRECT table name
  FOR item_record IN 
    SELECT * FROM public.management_event_line_items 
  ...
  LOOP
    ...
    BEGIN
      ...
      line_items_content := line_items_content || 
        '• ' || UPPER(item_record.type) || ': ' || item_record.description ||
