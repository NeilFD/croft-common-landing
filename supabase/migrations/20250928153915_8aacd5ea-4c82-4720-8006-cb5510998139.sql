-- Fix format() function calls in generate_contract to handle numeric values properly
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      -- Create display text for quantity
      line_qty_display := CASE 
        WHEN item_record.per_person THEN ROUND(line_qty)::text || ' people'
        ELSE ROUND(line_qty)::text
      END;
      ...
      -- Build line item text using simple concatenation instead of format()
      line_items_content := line_items_content || 
        '• ' || UPPER(item_record.type) || ': ' ||
        line_qty_display || ' x £' || ROUND(COALESCE(item_record.unit_price,0), 2)::text ||
        ' = £' || ROUND(line_total, 2)::text ||
  ...
  -- Create contract content using simple concatenation
  contract_content := 
    ...
    'SERVICES & PRICING' || E'\n' ||
    line_items_content ||
    'Subtotal (Net): £' || ROUND(total_net, 2)::text || E'\n' ||
    'VAT: £' || ROUND(total_vat, 2)::text || E'\n' ||
    'Service Charge: £' || ROUND(service_charge, 2)::text || E'\n' ||
    'TOTAL: £' || ROUND(final_total, 2)::text || E'\n';
