-- Add enhanced hierarchical logic support to get_advanced_member_analytics function
-- This creates a new version that supports master logic and section-level logic
...
BEGIN
  ...
  filtered_data AS (
    ...
    WHERE 
      -- Apply all individual filter logic using hierarchical approach
      (
        -- If master logic is 'all_sections', all active sections must match
        -- If master logic is 'any_section', at least one active section must match
        CASE 
          WHEN p_master_logic = 'all_sections' THEN
            -- All sections with filters must match
            (
              -- Activity section
              (
          ...
          ELSE
            -- Any section with filters must match (OR logic between sections)
            (
              ...
              -- Always apply spending and tier filters as they're core
              (
                (p_min_spend IS NULL OR ed.total_spend >= p_min_spend) AND
                (p_max_spend IS NULL OR ed.total_spend <= p_max_spend) AND
                (p_tier_badges IS NULL OR ed.tier_badge = ANY(p_tier_badges))
