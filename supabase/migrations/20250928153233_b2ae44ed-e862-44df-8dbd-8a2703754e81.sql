-- Fix the generate_contract function to handle missing company field
...
DECLARE
  ...
  item_record RECORD;
  company_name TEXT := 'Private Client'; -- Default fallback
BEGIN
  ...
  -- Try to get company name from leads table if lead_id exists
  IF event_data.lead_id IS NOT NULL THEN
    SELECT company INTO company_name 
    FROM public.leads 
    WHERE id = event_data.lead_id;
    
    -- If no company in leads, use client_name or fallback
    IF company_name IS NULL THEN
      company_name := COALESCE(event_data.client_name, 'Private Client');
    END IF;
  ELSE
    -- Use client_name as company or fallback
    company_name := COALESCE(event_data.client_name, 'Private Client');
  END IF;
...
', 
    ...
    company_name,  -- Use the resolved company name
