-- Update get_advanced_member_analytics function to support AND/OR logic for interests
...
RETURNS TABLE(user_id uuid, first_name text, last_name text, display_name text, age integer, interests text[], tier_badge text, total_transactions bigint, total_spend numeric, avg_transaction numeric, first_transaction_date date, last_transaction_date date, active_months bigint, active_days bigint, categories text[], payment_methods text[], currency text, current_month_spend numeric, current_week_spend numeric, current_month_transactions bigint, favorite_venues text[], visit_frequency integer, last_visit_date date, preferred_visit_times text[], retention_risk_score integer, lifetime_value numeric)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WITH authenticated_members_with_receipts AS (
    -- Start with users who have receipts AND are authenticated users
  ...
  base_data AS (
    ...
    WHERE 1=1
      ...
      -- Updated interests filter logic
      AND (p_interests IS NULL OR 
           CASE 
             WHEN p_interests_logic = 'match_all' THEN p.interests @> p_interests
             ELSE p.interests && p_interests
           END)
    ...
    HAVING 
      ...
      -- Only include members with actual transactions
      AND COUNT(mr.id) > 0
  ),
  enhanced_data AS (
    SELECT 
      ...
      -- Fixed lifetime value calculation: annualized based on spending rate
      CASE 
