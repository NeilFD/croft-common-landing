-- Correct JWT user resolution in SECURITY DEFINER get_chat_messages
...
DECLARE
  ...
  v_claims text;
BEGIN
  -- Resolve user id from JWT claims safely
  ...
  EXCEPTION WHEN others THEN
    v_user_id := NULL; -- parsing failure
  END;

  IF v_user_id IS NULL THEN
    RETURN; -- unauthenticated or bad token
  END IF;
