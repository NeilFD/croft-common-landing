-- Fix day-of-week calculation in get_member_deep_dive function
...
BEGIN
  ...
  -- Get most likely day of week with corrected day mapping
  WITH day_analysis AS (
    SELECT 
      -- Convert PostgreSQL DOW (0=Sunday) to readable format
  ...
  -- Get weekly pattern data with corrected day mapping
  SELECT jsonb_agg(
    jsonb_build_object(
      'day_of_week', day_num,
