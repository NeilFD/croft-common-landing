...
-- First, clean up duplicate ledger entries for receipts
-- Keep only the most recent ledger entry for each receipt
...
-- Add unique constraint to prevent future duplicate ledger entries per receipt
-- Use simpler constraint syntax for PostgreSQL
...
BEGIN
  ...
  -- Get time of day categorization using RECEIPT DATES (assuming morning/afternoon/evening based on general patterns)
  -- Since receipts don't have time stamps, we'll use day of week patterns instead
  SELECT jsonb_agg(
    jsonb_build_object(
  ...
  normalized_items AS (
    SELECT 
      original_name,
      -- Simple normalization since normalize_item_name function doesn't exist
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      'item_name', (SELECT unnest(all_names) LIMIT 1), -- Get first name as representative
