-- Fix get_member_deep_dive function - simplify weekly streak calculation
...
DECLARE
  ...
  total_weeks_complete INTEGER := 0;
BEGIN
  ...
  -- Calculate weekly streaks from receipt data (simplified)
  ...
  completed_weeks AS (
    SELECT week_start, receipt_count >= 2 as is_complete
    FROM weekly_counts
    ORDER BY week_start DESC
  )
  SELECT 
    COUNT(*) FILTER (WHERE is_complete) INTO total_weeks_complete
  ...
  WITH recent_weeks AS (
    ...
    FROM public.member_receipts mr
    WHERE mr.user_id = p_user_id
      AND mr.receipt_date >= CURRENT_DATE - INTERVAL '12 weeks'
    GROUP BY DATE_TRUNC('week', mr.receipt_date)::DATE
    ORDER BY week_start DESC
  )
  SELECT 
    COALESCE(
      (SELECT COUNT(*) 
       ...
       (SELECT COUNT(*) FROM recent_weeks WHERE is_complete = true)
    ), 0
  ...
  FROM (
    WITH RECURSIVE streak_calc AS (
      SELECT 
        week_start,
        receipt_count >= 2 as is_complete,
        CASE WHEN receipt_count >= 2 THEN 1 ELSE 0 END as streak_length,
        1 as position
      FROM (
        SELECT 
          DATE_TRUNC('week', mr.receipt_date)::DATE as week_start,
      ...
      SELECT 
        wc.week_start,
        wc.receipt_count >= 2 as is_complete,
        CASE 
          WHEN wc.receipt_count >= 2 AND sc.is_complete THEN sc.streak_length + 1
          WHEN wc.receipt_count >= 2 THEN 1
          ELSE 0
        END,
        sc.position + 1
      FROM (
      ...
      JOIN streak_calc sc ON wc.week_start > sc.week_start
      WHERE sc.position < 100  -- Prevent infinite recursion
    )
  ...
  -- Get visit patterns with corrected weekly streaks
  SELECT jsonb_build_object(
