-- Update get_member_deep_dive function to use receipt dates instead of upload times
...
BEGIN
  ...
  -- Get monthly trend data using RECEIPT DATES
  SELECT jsonb_agg(
    jsonb_build_object(
      'month', month_start,
      'spend', monthly_spend,
  ...
  -- Get recent activity using RECEIPT DATES
  SELECT ARRAY_AGG(
    jsonb_build_object(
      'date', activity_date,
      'type', activity_type,
