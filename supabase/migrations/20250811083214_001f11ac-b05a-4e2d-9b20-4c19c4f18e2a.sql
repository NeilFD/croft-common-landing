...
-- Safely try to unschedule without failing if it doesn't exist
DO $$
BEGIN
  PERFORM cron.unschedule('process-scheduled-notifications-every-minute');
EXCEPTION WHEN OTHERS THEN
  -- ignore if job doesn't exist
  NULL;
END $$;

-- Schedule the processor to run every minute (UTC)
select
  cron.schedule(
