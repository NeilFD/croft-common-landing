...
const Email = ({
  bookingId, slotDate, slotStart, slotEnd, guestFirstName, guestLastName,
  guestEmail, partySize, cancelledReason,
}: Props) => {
  const fullName = `${guestFirstName || ''} ${guestLastName || ''}`.trim()
  return (
    <Html lang="en" dir="ltr">
      <Head />
      <Preview>Cancelled — {fullName} — {fmtDate(slotDate)} {trimTime(slotStart)}</Preview>
      <Body style={main}>
        <Container style={container}>
          ...
          <Section style={card}>
            ...
            <Row label="Reason" value={cancelledReason || '—'} />
            <Row label="Booking ID" value={bookingId || '—'} />
          </Section>

          <Text style={textBlock}>Room is now free again for the public booking flow.</Text>

          <Hr style={hr} />
          <Text style={footer}>Auto-sent from crazybear.app.</Text>
        </Container>
      </Body>
    </Html>
  )
...
export const template = {
  ...
  subject: (d: Record<string, any>) =>
    `[Cancelled] Karaoke — ${d?.slotDate || ''} ${String(d?.slotStart || '').slice(0,5)} — ${d?.guestFirstName || ''} ${d?.guestLastName || ''}`.trim(),
  displayName: 'Karaoke — venue cancellation',
  previewData: {
    bookingId: 'abc-123', slotDate: '2026-06-14', slotStart: '18:00', slotEnd: '20:00',
    guestFirstName: 'Sam', guestLastName: 'Lee', guestEmail: 'sam@example.com',
    partySize: 8, cancelledReason: 'Plans changed',
  },
