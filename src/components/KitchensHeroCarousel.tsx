...
const KitchensHeroCarousel = () => {
  ...
  useEffect(() => {
    ...
    const initialCount = Math.min(kitchenImages.length, 6);
    const initial = kitchenImages.slice(0, initialCount);
    ...
    const handle = ric(() => {
      kitchenImages.slice(initialCount).forEach((img) => {
  ...
  useEffect(() => {
    if (!kitchenImages?.length) return;
    const total = kitchenImages.length; const next1 = (currentSlide + 1) % total; const next2 = (currentSlide + 2) % total;
    [next1, next2].forEach((idx) => {
      const src = kitchenImages[idx]?.src; if (!src || decodedSetRef.current.has(src)) return;
