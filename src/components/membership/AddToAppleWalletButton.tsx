 ...
 * Triggers the create-cb-wallet-pass edge function and downloads the .pkpass file.
 */
export const AddToAppleWalletButton: React.FC = () => {
  const { user } = useAuth();
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      // iOS Safari: opening the URL directly lets the system hand the .pkpass to Wallet.
  ...
  return (
    <div className="flex flex-col gap-2">
      <button
        ...
        onClick={handleAdd}
        disabled={loading}
        className="group flex items-center justify-center gap-3 w-full bg-black text-white border-2 border-black hover:bg-white hover:text-black transition-colors h-12 px-5 font-mono uppercase tracking-[0.3em] text-[11px] disabled:opacity-50 disabled:cursor-wait"
      ...
      >
        ...
        <span>{loading ? 'Preparing…' : 'Add to Apple Wallet'}</span>
      </button>
